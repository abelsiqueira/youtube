...
# ╔═╡ b5566f57-1aac-42dc-89ed-79a0e948795a
using PrettyTables, JuMP, Cbc, PlutoUI, Plots, LinearAlgebra

# ╔═╡ bb186f30-ef53-11ed-3b49-b9cf8b30ab4c
md"""
# Selling your art on the beach

**A mathematical modeling story**
...
# ╔═╡ df1e6376-5c97-4ab7-a05e-50e6db8afe9a
md"""
...
For each product, you have a different use of materials, a different selling price, and a different time to assemble them, as described in the table below:
"""

# ╔═╡ b9bdde07-0cd6-4190-92a7-3fc03845b1ab
begin
	...
	pretty_table(
		HTML,
		[material_per_product selling_price hours_to_assemble],
		header=[materials; "price"; "hours"] .* units,
		row_labels=products,
	)
end

# ╔═╡ 08252ffc-694d-4843-a659-c2e49fb15d63
md"""
Additionally, there are costs for the materials.
"""

# ╔═╡ 95872e39-a96d-43c6-8ee3-690f9d89bb2b
begin
...
# ╔═╡ 5fef189d-d525-41c2-85d4-6ca77707dc1a
md"""
Finally, Javier assumes that having variety is good for business, so he want at least some amount of each product.
"""

# ╔═╡ 48727e3f-febf-4f2c-a277-87039f2f6c50
begin
	min_prod = [2, 2, 2]
	...
	pretty_table(
		...
		min_prod',
		header=products,
		row_labels=["minimum production"],
	)
end

# ╔═╡ 1914c98a-bc11-4a4f-82b5-8cdf5dc00dfb
minimum_required_time = sum(min_prod .* hours_to_assemble)

# ╔═╡ 4482fb99-d170-40ab-bf17-e0bdbe7e4ae5
md"""
## Modeling
...
- Revenue:
$\displaystyle \text{revenue} = \sum_{p \in \mathcal{P}} \text{selling\_price}_p \times \text{prod}_p$
- Total Used Material:
...
- Minimum production:
$\text{prod}_p \geq \text{min\_prod}_p, \quad p \in \mathcal{P}$
- Integer production:
$\text{prod}_p \in \mathbb{Z}$
"""

# ╔═╡ 02f10634-74f9-436b-8a32-6d63bda26c7e
products

# ╔═╡ d058c09f-b434-4ded-ac11-92ae312d4d84
begin
	# using JuMP, Cbc
...
# ╔═╡ c6349108-bc5e-4745-9d11-197482dd21a1
md"""
Time Availability = $(@bind slider_time_availability Slider(ceil(Int, minimum_required_time):40, show_value=true))
"""

# ╔═╡ c0ba129e-51a6-462c-8ed8-6e0c62e8b9da
let
	production, _ = optimize_production(slider_time_availability)
	pretty_table(
		HTML,
		production',
		header=products,
		row_labels=["production"],
	)
end

# ╔═╡ 848b3a22-0ff0-4539-b6bb-d9f629c292d0
let
	time_availabilities = ceil(Int, minimum_required_time):40
	productions = getfield.(optimize_production.(time_availabilities), 1)
	revenues = dot.(productions, Ref(selling_price))
	costs = dot.(productions, Ref(material_per_product), Ref(material_cost))
	profits = revenues - costs
	slacks = dot.(productions, Ref(hours_to_assemble))

	plt = plot(layout=grid(1, 2), size=(800, 400))
	bar!(
		plt[1],
		time_availabilities,
		time_availabilities - slacks,
		lab="Unnalocated time (h)",
	)
	plot!(
		plt[2],
		time_availabilities,
...
# ╔═╡ 2e4475fd-4386-409e-929b-20d5092a83d6
let
	production, profit, model = optimize_production(slider_time_availability)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "a4291603974cdf38bc5df2eecfd306c370102365"
...
git-tree-sha1 = "55e2e9684102dd2f68c406a6a282b16883a7bc00"
